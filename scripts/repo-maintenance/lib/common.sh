...
if [ -n "${SELF_DIR:-}" ] && [ -d "$SELF_DIR/lib" ]; then
  COMMON_DIR=$(CDPATH= cd -- "$SELF_DIR/lib" && pwd)
elif [ -n "${SELF_DIR:-}" ] && [ -d "$SELF_DIR/../lib" ]; then
  COMMON_DIR=$(CDPATH= cd -- "$SELF_DIR/../lib" && pwd)
else
...
resolve_repo_root() {
  if git -C "$REPO_MAINTENANCE_ROOT" rev-parse --show-toplevel >/dev/null 2>&1; then
    git -C "$REPO_MAINTENANCE_ROOT" rev-parse --show-toplevel
    return 0
  fi

  CDPATH= cd -- "$REPO_MAINTENANCE_ROOT/../.." && pwd
}
